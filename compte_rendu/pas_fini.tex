...
\paragraph{Calcul des probabilités}
	...
	Soit $X, Y$ deux variables aléatoires réelles sur $\Omega$ et à valeur dans $\llbracket 0, n \rrbracket, n \in \mathbb{N}$, telles que $\forall k \in \llbracket 0, n \rrbracket, X$ sachant que $Y = k$ suit une loi binomiale de paramètres $n-k, p, p \in \left]0, 1\right[$. Alors $E(X) = (n - E(Y))p$.\\
	...
	Au jour $k$ il reste $$n - \sum_{i = 0}^{k-1} X_i$$ individus sains. Montrons par récurrence que $\forall k \in \llbracket 1, \tau \rrbracket, E(X_k) = n(1-p)^{k-1}p$\\
	...
	Ou bien $\tau \ge 2$ et on suppose que $P(k)$ est vraie pour $k \in \in \llbracket 1, \tau - 1 \rrbracket$. Montrons que $P(k+1)$ est vraie.\\
